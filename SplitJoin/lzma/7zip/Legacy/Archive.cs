...
namespace master._7zip.Legacy
{
    ...
    {
        public static string[] MustDelete = new string[0];
    ...
    public class ArchiveReader
    {
                ...
                for (int i = 0; i < numCoders; i++)
                {
                    ...
                    try
                    {
                        ...
                        if ((mainByte & 0x20) != 0)
                        {
                            ...
                            string[] test = ArrayUtility.ArrayUtil.Select(coder.Props, delegate (Byte f) { return f.ToString("x2"); });
                            Log.WriteLine("Settings: " + test);
                        }
        ...
        private Stream GetCachedDecoderStream(CArchiveDatabaseEx _db, int folderIndex, IPasswordProvider pw)
        {
            ...
            if (!_cachedStreams.TryGetValue(folderIndex, out s))
            {
                ...
                if (!s.CanSeek) //JOHN
                {
                    string temp = Path.GetTempFileName() + "_" + ArrayUtility.MyRandom.RandomStringLower(8);
                    CArchiveDatabaseEx.MustDelete = ArrayUtility.ArrayUtil.AddArray(CArchiveDatabaseEx.MustDelete, temp);
                    FileStream ms = new FileStream(temp, FileMode.CreateNew, FileAccess.ReadWrite, FileShare.Delete, 1024 * 100);

                    s.CopyTo(ms);
                    ms.Position = 0;
                    s = new ManagedLzma._7zip.Decoder.FileBufferedDecoderStream(ms);
                }
