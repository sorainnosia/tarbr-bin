...
namespace ManagedLzma.SevenZip.Writer
{
    ...
    public sealed class ArchiveWriter : IDisposable
    {
        ...
        public List<bool> SelectMetadata(ArchiveMetadataProvider metadata, int metadataCount)
        {
            List<bool> result = new List<bool>();
            for (int i = 0; i < metadataCount; i++)
                if (metadata.HasStream(i) == false) result.Add(metadata.HasStream(i));
            return result;
        }


        public List<bool> SelectMetadataNotDirectory(ArchiveMetadataProvider metadata, int metadataCount)
        {
            List<bool> result = new List<bool>();
            for (int i = 0; i < metadataCount; i++)
                if (metadata.HasStream(i) == false) result.Add(metadata.IsDirectory(i));
            return result;
        }
        public List<bool> SelectMetadataDeleted(ArchiveMetadataProvider metadata, int metadataCount)
        {
            List<bool> result = new List<bool>();
            for (int i = 0; i < metadataCount; i++)
                if (metadata.HasStream(i) == false) result.Add(metadata.IsDeleted(i));
            return result;
        }
            ...
            if (subStreamCount > 0)
            {
                ...
                #region Types
                {
                    ...
                    if (emptyStreamCount > 0)
                    {
                        WriteBitVectorWithHeader(ArchiveMetadataToken.EmptyStream,
                            ...
                            SelectMetadata(metadata, metadataCount),
                            metadataCount);
                        ...
                        if (Enumerable.Range(0, metadataCount).Where(x => !metadata.HasStream(x)).Any(x => !metadata.IsDirectory(x)))
                            WriteBitVectorWithHeader(ArchiveMetadataToken.EmptyFile,
                                ...
                                SelectMetadataNotDirectory(metadata, metadataCount),
                                emptyStreamCount);
                        ...
                        if (Enumerable.Range(0, metadataCount).Where(x => !metadata.HasStream(x)).Any(x => metadata.IsDeleted(x)))
                            WriteBitVectorWithHeader(ArchiveMetadataToken.Anti,
                                ...
                                SelectMetadataDeleted(metadata, metadataCount),
                                emptyStreamCount);
                    }
                }
                #endregion
                ...
                #region Names
                {
                    ...
                    if (hasNames)
                    {
                        ...
                        ManagedLzma.LZMA.Master.SevenZip.Utils.Assert((mArchiveStream.Position & 15) == 0);
                ...
                List<DateTime?> lastaccess = new List<DateTime?>();
                List<DateTime?> writedate = new List<DateTime?>();
                List<ulong?> youlong = new List<ulong?>();
                List<uint?> youint = new List<uint?>();
                for (int i = 0; i < metadataCount; i++)
                {
                    creationdate.Add(metadata.GetCreationDate(i));
                    lastaccess.Add(metadata.GetLastAccessDate(i));
                    writedate.Add(metadata.GetLastWriteDate(i));

                    var attr = metadata.GetAttributes(i);
                    if (attr.HasValue)
                        youint.Add((uint)attr.Value);
                    else
                        youint.Add(default(uint?));
                    youlong.Add(default(ulong?));
                }
                ...
                WriteDateVector(creationdate, ArchiveMetadataToken.CTime);
                WriteDateVector(lastaccess, ArchiveMetadataToken.ATime);
                WriteDateVector(writedate, ArchiveMetadataToken.MTime);
                ...
                WriteUInt64Vector(youlong, ArchiveMetadataToken.StartPos);
                ...
                WriteUInt32Vector(youint, ArchiveMetadataToken.WinAttributes);
                WriteToken(ArchiveMetadataToken.End);
            }
        ...
        private void WriteChecksumVector(IEnumerable<Checksum?> checksums)
        {
            if (checksums.Any(x => x.HasValue))
            {
                ...
                {
                    WriteByte(0);
                    bool[] hasvalues = ArrayUtility.ArrayUtil.Select(checksums, delegate (Checksum? f) { return f.HasValue; });
                    //WriteBitVector(checksums.Select(x => x.HasValue));
                    WriteBitVector(hasvalues);
                }
        ...
        {
            ulong?[] hasvalues = ArrayUtility.ArrayUtil.Select(dates, delegate (DateTime? f) { return f.HasValue ? (ulong)f.Value.ToFileTimeUtc() : default(ulong?); });
            //WriteUInt64Vector(dates.Select(x => x.HasValue ? (ulong)x.Value.ToFileTimeUtc() : default(ulong?)), token);
            WriteUInt64Vector(hasvalues, token);
        }
        ...
        private void WriteUInt64Vector(IEnumerable<ulong?> vector, ArchiveMetadataToken token)
        {
            ...
            {
                List<bool> result = new List<bool>();
                IEnumerator<ulong?> venum = vector.GetEnumerator();
                while (venum.MoveNext())
                    if (venum.Current.HasValue) result.Add(venum.Current.HasValue);

                WriteAlignedHeaderWithBitVector(result, count, defined, token, 8);
                ManagedLzma.LZMA.Master.SevenZip.Utils.Assert((mArchiveStream.Position & 7) == 0);
        ...
        private void WriteUInt32Vector(IEnumerable<uint?> vector, ArchiveMetadataToken token)
        {
            ...
            {
                List<bool> result = new List<bool>();
                IEnumerator<uint?> venum = vector.GetEnumerator();
                while (venum.MoveNext())
                    if (venum.Current.HasValue) result.Add(venum.Current.HasValue);

                WriteAlignedHeaderWithBitVector(result, count, defined, token, 4);
                ManagedLzma.LZMA.Master.SevenZip.Utils.Assert((mArchiveStream.Position & 3) == 0);
        ...
        private void WritePackInfo()
        {
            ...
            {
#if DEBUG
                ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(mFileSections[0].Offset == ArchiveMetadataFormat.kHeaderLength);
                for (int i = 1; i < mFileSections.Count; i++)
                    ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(mFileSections[i].Offset == mFileSections[i - 1].Offset + mFileSections[i - 1].Length);
#endif
                ...
                Checksum?[] hasvalues = ArrayUtility.ArrayUtil.Select(mFileSections, delegate (ArchiveFileSection f) { return f.Checksum; });
                //WriteChecksumVector(mFileSections.Select(x => x.Checksum));
                WriteChecksumVector(hasvalues);
        ...
        private void WriteUnpackInfo()
        {
            if (mDecoderSections.Count > 0)
            {
                ...
                Checksum?[] hasvalues = ArrayUtility.ArrayUtil.Select(mDecoderSections, delegate (ArchiveDecoderSection f) { return f.Checksum; });
                WriteChecksumVector(hasvalues);
        ...
        private void WriteDecoderSection(ArchiveDecoderSection definition, ref int firstStreamIndex)
        {
            ...
            for (int i = 0; i < definition.Decoders.Length; i++)
            {
                ...
                ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(!id.IsDefaultOrEmpty && id.Length <= 15);
        ...
        {
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(0 <= (int)token && (int)token <= 25);
        ...
        {
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(value >= 0);
        ...
        internal void CompleteEncoderSession(EncoderSession session, int section, ArchiveDecoderSection definition, EncoderStorage[] storageList)
        {
            ...
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(mDecoderSections[section] == null);
    ...
    public sealed class ArchiveMetadataRecorder : ArchiveMetadataProvider
    {
        ...
        private void CheckDate(ref DateTime? date)
        {
            if (date.HasValue)
            {
                ...
                ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(date.Value.Kind == DateTimeKind.Utc);
            }
        }
