            ...
            foreach (var section in sections)
            {
                ...
                for (int i = 0; i < subsections.Length - 1; i++)
                {
                    if (token == ArchiveMetadataToken.Size)
                    {
                        ...
                        {
                            subsections[i] = new DecodedStreamMetadata(0, null);
                        }
                    }
                }
                ...
                else if (token == ArchiveMetadataToken.CRC)
                {
                    ...
                    foreach (var section in sections)
                    {
                        if (section.SubStreamCount == 1 && section.OutputChecksum.HasValue)
                        {
                            ...
                            section.Subsections[0].Checksum = checksums[checksums.Count - 1];
                        }
                        else
                        {
                            for (int i = 0; i < section.SubStreamCount; i++)
                            {
                                ...
                                section.Subsections[i].Checksum = checksums[checksums.Count - 1];
                            }
                        }
                    }
                    ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(requiredChecksumIndex == requiredChecksumCount);
                    ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(checksums.Count == totalChecksumCount);
                }
                else
                {
                    SkipDataBlock();
