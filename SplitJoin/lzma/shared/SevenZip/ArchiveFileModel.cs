...
namespace ManagedLzma.SevenZip.FileModel
{
    ...
    public sealed class ArchiveFileModelMetadataReader : ArchiveMetadataReader
    {
                ...
                for (int currentFileIndex = 0; currentFileIndex < mFileNames.Count; currentFileIndex++)
                {
                    ...
                    if (file != null)
                    {
                        ...
                        if (hasStream)
                        {
                            ...
                            //if (file.Checksum == null) file.Checksum = currentDecoder.Checksum;
                        }
                    }
        ...
        private ArchivedFile.Builder AddItem(List<ArchivedItem.Builder> items, string fullName, int offset, bool isFile)
        {
            ...
            if (ending < 0)
            {
                ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(!isFile);
                return null;
            }
        ...
        {
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(builder.Items.Count == 0);
        ...
        {
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(mFileNames == null);
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(data.Count == mItemCount);
        ...
        {
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(mItemsWithoutStreamMarkers == null);
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(data.Count == mItemCount);
        ...
        {
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(mEmptyFileMarkers == null);
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(data.Count == mItemsWithoutStream);
        ...
        {
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(mDeletionMarkers == null);
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(data.Count == mItemsWithoutStream);
        ...
        {
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(mOffsets == null);
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(data.Count == mItemCount);
        ...
        {
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(mAttributes == null);
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(data.Count == mItemCount);
        ...
        {
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(mCDates == null);
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(data.Count == mItemCount);
        ...
        {
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(mMDates == null);
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(data.Count == mItemCount);
        ...
        {
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(mADates == null);
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(data.Count == mItemCount);
