...
namespace ManagedLzma.SevenZip.Writer
{
    ...
    internal sealed class AesEncoderNode : EncoderNode
    {
        private sealed class EncryptionStream : IStreamWriter
        {
            ...
            {
                ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(mOutput == null && output != null);
                mOutput = output;
            }
            ...
            public async Task<int> WriteAsync(byte[] buffer, int offset, int length, StreamMode mode)
            {
                ...
                for (; ; )
                {
                    if (mOffset == 16)
                    {
                        ...
                        ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(written == 16);
                        mOffset = 0;
                    }

                    int copy = Math.Min(16 - mOffset, length);
                    ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(copy > 0);
            ...
            public async Task CompleteAsync()
            {
                if (mOffset != 0)
                {
                    ...
                    ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(mOffset == 16);
                ...
                ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(mOffset == 0);
