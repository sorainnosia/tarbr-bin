...
namespace ManagedLzma.SevenZip.Writer
{
    ...
    internal sealed class EncoderInput : IStreamReader
    {
        ...
        public void Dispose()
        {
            ...
            {
                //System.Diagnostics.Debugger.Break();//JOHN
                throw new Exception("DebuggerBreak");
            }
        }
        ...
        public Checksum? GetFinalChecksum()
        {
            ...
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(mChecksum != CRC.kInitCRC || mLength == 0);
            return new Checksum((int)CRC.Finish(mChecksum));
        }

        public void SetInputStream(EncoderNode encoder, int index)
        {
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(mEncoderInput == null);
        ...
        private async Task TransferLoop()
        {
            ...
            for (; ; )
            {
                ...
                ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(0 <= fetched && fetched <= buffer.Length);
                if (fetched == 0)
                {
                ...
                ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(written == fetched);
    ...
    internal sealed class EncoderStorage : IStreamWriter
    {
        ...
        {
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(mEncoderOutput == null);
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(!mCompletionTask.Task.IsCompleted);
    ...
    internal sealed class EncoderConnection : IStreamReader, IStreamWriter
    {
        ...
        private async Task TransferLoop()
        {
            ...
            for (; ; )
            {
                if (!mComplete && ending < buffer.Length)
                {
                    ...
                    ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(0 <= fetched && fetched <= buffer.Length - ending);
                ...
                if (offset < ending)
                {
                    ...
                    ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(0 < written && written <= ending - offset);
        ...
        {
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(mConnectionOutputToEncoderInput == null);
        ...
        {
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(mEncoderOutputToConnectionInput == null);
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(buffer != null);
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(0 <= offset && offset < buffer.Length);
            ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(0 < count && count <= buffer.Length - offset);
        ...
        public async Task Complete()
        {
            ...
            for (int i = 0; i < mEncoders.Length; i++)
            {
                ...
                for (int j = 0; j < decoderOutputCount; j++)
                {
                    ...
                    {
                        ManagedLzma.LZMA.Master.SevenZip.Utils.Assert(mConnections[firstInputOffset[i] + j] == null);
