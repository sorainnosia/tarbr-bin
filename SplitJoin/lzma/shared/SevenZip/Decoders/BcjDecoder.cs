        ...
        private int DecodeInto(byte[] buffer, int offset, int count)
        {
            ...
            unsafe
            {
                fixed (byte* pBuffer = mBuffer)
                {
                    ...
                    if (delta == 0)
                    {
                        throw new NotSupportedException(); //JOHN
                                                           //if (Math.Min(mEnding - mOffset, count) < 5)
                                                           //{
                                                           //    Buffer.BlockCopy(mBuffer, mOffset, mBuffer, offset, Math.Min(mEnding - mOffset, count));
