...
//private static void Add7ZipDir(ArchiveWriter writer, string sourceDirectory, string currentdirectory)
//{
//    var pathToCurrentDirectory = Path.Combine(sourceDirectory, currentdirectory);
//    if (string.IsNullOrEmpty(sourceDirectory))
//    {
...
//        writer.WriteFile(name.Replace("\\", "/"), file);
//    }

//    var directories = Directory.GetDirectories(pathToCurrentDirectory);
//    foreach (string directoryx in directories)
//    {
//        Add7ZipDir(writer, sourceDirectory, directoryx.Substring(sourceDirectory.Length + 1));
//    }
//}
...
//    using (var stream = new FileStream(ot, FileMode.Create, FileAccess.ReadWrite, FileShare.Delete, filebuffer))
//    using (var encryption = new AESEncryptionProvider(password))
...
//            if (Directory.Exists(input[i]))
//            {
//                Add7ZipDir(writer, Path.GetDirectoryName(input[i]) ?? "", Path.GetFileName(input[i]));
//            }
//            else if (File.Exists(input[i]))
//            {
//                FileInfo file = new FileInfo(input[i]);
//                writer.WriteFile(Path.GetFileName(input[i]).Replace("\\", "/"), file);
//            }
//        }
...
//private static string Un7Zip(string input, string output, string password, string[] specificfile)
//{
//    if (File.Exists(input) == false)
//    {
//        Console.WriteLine("UN7Z: File '" + input + "' does not exists");
//        return String.Empty;
//    }

//    string destFolder = Path.GetFullPath(input);
//    if (destFolder.ToUpper().EndsWith(".7Z"))
//        destFolder = destFolder.Substring(0, destFolder.Length - 3);
//    if (destFolder.IndexOf(".") > 0 && destFolder.LastIndexOf(".") >= destFolder.Length - 5 && (File.Exists(destFolder) || Directory.Exists(destFolder)))
//        destFolder = destFolder.Substring(0, destFolder.LastIndexOf("."));

//    if (string.IsNullOrEmpty(output) == false) destFolder = output;
//    if (Directory.Exists(destFolder) == false) Directory.CreateDirectory(destFolder);

//    //ZipFile.ExtractToDirectory(input, destFolder);

//    string result = UnpackArchive(input, destFolder, password, specificfile);
//    return result;
//}

//private static string UnpackArchive(string archiveFileName, string targetDirectory, string password, string[] specificfile)
//{
//    ManagedLzma.PasswordStorage pass = null;
//    if (string.IsNullOrEmpty(password) == false) pass = ManagedLzma.PasswordStorage.Create(password);
...
//        ArchiveFileModel archiveFileModel = null;
//        if (password != null)
//            archiveFileModel = archiveMetadataReader.ReadMetadata(archiveStream, pass);
//        else
//            archiveFileModel = archiveMetadataReader.ReadMetadata(archiveStream);
...
//            {
//                var sectionReader = new ManagedLzma.SevenZip.Reader.DecodedSectionReader(archiveStream, archiveMetadata, sectionIndex, pass);
...
//                    if (specificfile == null || specificfile.Length == 0 || ArrayUtil.ArrayContains(specificfile, fileMetadata.FullName.Replace("\\", "/")))
//                    {
...
//                        using (var stream = new FileStream(filename, FileMode.Create, FileAccess.Write, FileShare.None, filebuffer))
//                        {
//                            try
//                            {
//                                using (Stream str = sectionReader.OpenStream())
//                                {
//                                    str.CopyTo(stream);
//                                }
//                            }
...
//                        //int pInput = 0;
//                        //int pOutput = 0;
//                        //using (var decoder = new ManagedLzma.LZMA2.Decoder(new ManagedLzma.LZMA2.DecoderSettings(12)))
//                        //{
//                        //    do
//                        //    {
...
//                        if (fileMetadata.Checksum.HasValue && pass != null)
//                        {
//                            var encoder = new ManagedLzma.SevenZip.Writer.EncoderDefinition();
//                            ManagedLzma.SevenZip.Writer.EncoderNodeDefinition node1 = null;
//                            ManagedLzma.SevenZip.Writer.EncoderNodeDefinition node2 = null;
//                            ManagedLzma.PasswordStorage passx = null;
//                            if (string.IsNullOrEmpty(password) == false) passx = ManagedLzma.PasswordStorage.Create(password);

//                            //node1 = encoder.CreateEncoder(new ManagedLzma.SevenZip.Writer.Lzma2EncoderSettings(new ManagedLzma.LZMA2.EncoderSettings()));
//                            if (string.IsNullOrEmpty(password) == false)
//                            {
//                                node2 = encoder.CreateEncoder(new ManagedLzma.SevenZip.Writer.AesEncoderSettings(passx));
//                            }
//                            else
//                            {
//                                //node2 = encoder.CreateEncoder(new ManagedLzma.SevenZip.Writer.AesEncoderSettings(ManagedLzma.PasswordStorage.Create("")));
...
//                            using (var archiveWriter = ManagedLzma.SevenZip.Writer.ArchiveWriter.Create(new MemoryStream(), false))
//                            {
//                                using (var session = archiveWriter.BeginEncoding(encoder, true))
//                                {
//                                    using (FileStream fileStream = new FileStream(filename, FileMode.Open, FileAccess.Read, FileShare.Read, filebuffer))
//                                    {
//                                        var result = session.AppendStream(fileStream, true).GetAwaiter().GetResult();
//                                        if (result.Checksum.HasValue)
//                                        {
//                                            if (fileMetadata.Checksum.Value != result.Checksum.Value)
//                                            {
...
//                                    session.Complete().GetAwaiter().GetResult();
//                                }
...
//                        if (specificfile != null && specificfile.Length > 0 && specificfile.Length == count)
//                            break;
//                    }
//                }
//                if (specificfile != null && specificfile.Length > 0 && specificfile.Length == count)
...
//        const FileAttributes kAttrMask = ManagedLzma.SevenZip.ArchivedAttributesExtensions.FileAttributeMask;
//        attr = (attr & ~kAttrMask) | (file.Attributes.Value.ToFileAttributes() & kAttrMask);
//        File.SetAttributes(path, attr);
//    }
//}

//using (TarArchive tarArchive = TarArchive.CreateInputTarArchive(inStream, System.Text.Encoding.ASCII))
//{
...
//public static string DecompressMany(string type, string[] input, string output, bool replace, Func<string, string, bool, string> func, out string[] result)
//{
//    result = new string[0];
//    string resultstr = string.Empty;
...
//    for (int i = 0; i < input.Length; i++)
//    {
//        if (File.Exists(input[i]) == false)
//        {
//            Console.WriteLine("UN" + type.ToUpper() + ": File '" + input[i] + "' does not exists");
//            failed++;
//            return resultstr;
//        }
//        else
...
//                tried++;
//                string file = input[i];
//                string ot = Path.GetFullPath(file);
//                if (file.ToUpper().EndsWith("." + type.ToUpper())) ot = file.Substring(0, file.Length - 3);
//                //if (string.IsNullOrEmpty(output) == false) ot = output;
//                string dir = Path.GetDirectoryName(ot);
//                CreateDir(dir);
//                if (IsAlreadyExist("UN" + type.ToUpper(), ot, replace))
//                {
//                    failed++;
//                    continue;
//                }
...
//                    result = ArrayUtil.AddArray(result, resultstr);
//                count++;
//            }
//            catch(Exception ex)
//            {
//                failed++;
//                Console.WriteLine(type.ToUpper() + ": Error file '" + input[i] + "', " + ex.Message);
//            }
//        }
//    }
...
//        else if ((task.Operation == "-SHA" || task.Operation == "-BR" || task.Operation == "-UNBR" || task.Operation == "-UNTAR" || task.Operation == "-UNZIP" || task.Operation == "-GZ" || task.Operation == "-UNGZ" || task.Operation == "-ZLIB" || task.Operation == "-UNZLIB" || task.Operation == "-UNTB" || task.Operation == "-UNTBZ" || task.Operation == "-UNTG" || task.Operation == "-UNTZ" || task.Operation == "-UN7Z") && task.Input.Length < 1)
//        {
//            if (task.Operation == "-JOIN")
//                Console.WriteLine(task.Operation.Substring(1) + ": Please specify a single filename (without .1) :");
//            else
//                Console.WriteLine(task.Operation.Substring(1) + ": Please specify a single filename :");
//            Console.WriteLine(task.Operation + " <filename>");
//            return;
//        }